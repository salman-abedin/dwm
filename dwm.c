...
void buttonpress(XEvent *e) {
   ...
   if (ev->window == selmon->barwin) {
      ...
      do x += TEXTW(occ & 1 << i ? alttags[i] : tags[i]);
   ...
   if ((c = wintoclient(ev->window))) {
      if (ev->value_mask & CWBorderWidth)
         ...
         if ((c->x + c->w) > m->mx + m->mw && c->isfloating)
            c->x =
                m->mx + (m->mw / 2 - WIDTH(c) / 2); /* center in x direction */
...
void manage(Window w, XWindowAttributes *wa) {
   ...
   c->y = MAX(
       c->y, ((c->mon->by == c->mon->my) && (c->x + (c->w / 2) >= c->mon->wx) &&
   ...
   for (i = 0, my = ty = gappx, c = nexttiled(m->clients); c;
      ...
      if (i < m->nmaster) {
         ...
         resize(c, m->wx + gappx, m->wy + my,
                mw - (2 * c->bw) - gappx * (5 - ns) / 2, h - (2 * c->bw),
                False);
         my += HEIGHT(c) + gappx;
      } else {
         h = (m->wh - ty) / (n - i) - gappx;
         resize(c, m->wx + mw + gappx / ns, m->wy + ty,
                m->ww - mw - (2 * c->bw) - gappx * (5 - ns) / 2,
                h - (2 * c->bw), False);
         ty += HEIGHT(c) + gappx;
      }
}
